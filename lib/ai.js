...
async function callGroq(model, messages, maxTokens = 2000) {
...
async function callWithFallback(messages) {
  ...
  // Fallback model has 6000 TPM limit: 1200 output + ~400 prompt = ~4400 left for content
  // 4400 tokens * 4 chars/token ≈ 8000 chars safe limit for fallback
  const FALLBACK_MAX_TOKENS = 1200;
  const FALLBACK_MAX_CHARS = 7000;

  const truncateMessages = (msgs, maxChars) => {
    return msgs.map((m) => {
  ...
  try {
    const result = await withTimeout(callGroq(primaryModel, messages, 2000));
    return result;
  } catch (err) {
    ...
    if (err.status === 429 || err.status === 413 || err.message === "Groq timeout") {
      ...
      const truncated = truncateMessages(messages, FALLBACK_MAX_CHARS);
      return withTimeout(callGroq(fallbackModel, truncated, FALLBACK_MAX_TOKENS));
    }
    throw err;
  }
}
