...
async function callGroq(prompt, text, chunkInfo = "") {
  ...
  try {
    const res = await withTimeout(makeCall(true), TIMEOUT_MS, "Groq AI");
    return JSON.parse(res.choices[0].message.content);
  } catch (err) {
    // Don't retry on timeout or rate limit — propagate immediately
    if (err.message?.includes("timed out") || isRateLimit(err)) {
      const msg = isRateLimit(err)
        ? "Groq API rate limit reached — please try again in a few minutes or upgrade your Groq plan at console.groq.com"
        : err.message;
      throw new Error(msg);
    }

    // Attempt 2: without JSON format (Groq occasionally rejects the format flag)
    try {
      const res = await withTimeout(makeCall(false), TIMEOUT_MS, "Groq AI (retry)");
      ...
      if (retryErr.message?.includes("timed out") || isRateLimit(retryErr)) throw retryErr;
      throw new Error(`AI extraction failed: ${retryErr.message}`);
    }
  }
}
