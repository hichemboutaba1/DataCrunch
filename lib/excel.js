...
export async function generateExcel(doc, extracted) {
  ...
  const severityColor = { HIGH: RED, MEDIUM: ORANGE, LOW: "FF95A5A6" };
  const severityLabel = { HIGH: "RISQUE ÉLEVÉ", MEDIUM: "RISQUE MODÉRÉ", LOW: "RISQUE FAIBLE" };

  const flags = [...(extracted._flags || [])].sort(
    (a, b) => (severityOrder[a.severity] ?? 9) - (severityOrder[b.severity] ?? 9)
  );

  let currentSeverity = null;
  for (const flag of flags) {
    if (flag.severity !== currentSeverity) {
      currentSeverity = flag.severity;
      const groupRow = flagSheet.addRow([severityLabel[flag.severity] || flag.severity, "", "", ""]);
      const gc = severityColor[flag.severity] || "FF999999";
      groupRow.eachCell((cell) => {
        cell.font = { bold: true, color: { argb: WHITE }, size: 10 };
        cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: gc } };
      });
      flagSheet.mergeCells(`A${groupRow.number}:D${groupRow.number}`);
    }
    const row = flagSheet.addRow([flag.severity, flag.category, flag.label, flag.detail]);
    const color = severityColor[flag.severity] || "FF999999";
