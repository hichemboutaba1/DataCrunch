...
function sum(items) {
  return items?.reduce((acc, item) => acc + (Number(item.amount) || 0), 0) || 0;
}
...
export function validateExtraction(extracted) {
  ...
  for (const key of ["revenue", "expenses", "assets", "liabilities"]) {
    if (doc[key]?.mismatch) {
      const diff = doc[key].total_calculated - doc[key].total_stated;
      issues.push(`${key}: stated=${doc[key].total_stated?.toLocaleString()} calculated=${doc[key].total_calculated?.toLocaleString()} diff=${diff > 0 ? "+" : ""}${diff.toLocaleString()}`);
    }
  }
  if (doc.mismatch) {
    const diff = doc.total_calculated - doc.total_stated;
    issues.push(`Total mismatch: diff=${diff > 0 ? "+" : ""}${diff?.toLocaleString()}`);
  }
